...
namespace trimana_engine::app
{
    ...
    public:
         ...
         */
        void push_layer(std::shared_ptr<trimana_core::layers::layer> layer);
         ...
         */
        void push_overlay(std::shared_ptr<trimana_core::layers::layer> overlay);
    ...
    private:
         ...
         * functionality.
         */
        std::shared_ptr<trimana_core::windows::window> m_window;

        /**
         * A smart pointer that manages the lifetime of an object of type `trimana_core::layers::layer_stack`.
         *
         * The `std::shared_ptr` class template is a smart pointer that retains shared ownership of an object through a pointer. Multiple `std::shared_ptr` objects can own the same object. The object is destroyed and its memory deallocated when either of the following happens:
         * - the last remaining `std::shared_ptr` owning the object is destroyed
         * - the last remaining `std::shared_ptr` owning the object is assigned another pointer via the `operator=` or `reset()` member functions
         *
        ...
        //TEMP
        std::shared_ptr<trimana_core::renderer::shader> m_shader_triangle;
        std::shared_ptr<trimana_core::renderer::vertex_array> m_vertex_array_triangle;

        std::shared_ptr<trimana_core::renderer::shader> m_shader_square;
        std::shared_ptr<trimana_core::renderer::vertex_array> m_vertex_array_square;
    };
}
